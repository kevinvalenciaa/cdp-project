...
export function OpportunityDetail({
  ...
  return (
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent side="right" className="flex w-full flex-col gap-0 overflow-y-auto bg-card p-0 sm:max-w-[640px]">
        <SheetHeader className="border-b border-border px-6 py-6 text-left">
          <div className="flex items-center gap-2">
            <StatusPill tone={o.accepted ? "emerald" : vm.tone}>{o.accepted ? "proven" : vm.label}</StatusPill>
            {o.accepted && <span className="text-sm font-semibold tabular-nums text-ht-green">{moneyCompact(monthlyImpact(o))}/mo est. impact</span>}
          </div>
          <SheetTitle className="mt-2 text-xl tracking-[-0.02em] text-foreground">{o.title}</SheetTitle>
        ...
        <Tabs defaultValue={o.accepted ? "plan" : "analysis"} className="flex flex-1 flex-col">
          <TabsList className="mx-6 mt-4 h-10 justify-start gap-1 rounded-xl bg-muted p-1">
            <TabsTrigger
              value="analysis"
              className="h-8 rounded-lg border-0 bg-transparent px-3 py-1 text-sm text-muted-foreground shadow-none data-[state=active]:bg-card data-[state=active]:text-foreground data-[state=active]:shadow-ht-xs"
            >
              Analysis
            </TabsTrigger>
            {o.accepted && (
          ...
          <TabsContent value="analysis" className="space-y-4 px-6 py-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {sourceChips(o).map((s) => (
                <span key={s} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1 text-[11px] text-muted-foreground">
                  {s}
                </span>
              ))}
            </div>
            {!o.accepted && o.bareLlm?.accepted && (
              <div className="rounded-2xl border border-ht-danger/20 bg-ht-danger-bg/60 p-4 text-xs text-muted-foreground">
          ...
          {o.accepted && (
            <TabsContent value="plan" className="space-y-4 px-6 py-5">
                ...
                <>
                  <section>
                    ...
                    <div className="rounded-2xl border border-border bg-background p-4 text-sm">
                      ...
                      <code className="mt-2 block rounded-xl bg-card px-2.5 py-1.5 font-mono text-[11px] text-muted-foreground">
                  ...
                  {activation.brief && (
                    <section>
                      ...
                      <div className="whitespace-pre-line rounded-2xl border border-border bg-background p-4 text-sm leading-relaxed text-muted-foreground">
              ...
              {result && (
                <div className="rounded-2xl border border-ht-green-border bg-ht-green-bg/60 p-4 text-sm">
              ...
              {!canActivate && (
                <div className="rounded-2xl border border-ht-warning/25 bg-ht-warning-bg p-4 text-sm text-ht-warning">
          ...
          {history.length > 0 && (
            <TabsContent value="history" className="space-y-3 px-6 py-5">
              ...
              {history.map((occurrence, index) => (
                <article key={occurrence.id} className="rounded-2xl border border-border bg-background p-4">
                  ...
                  <Link
                    href={`/investigations/${occurrence.investigationId}`}
        ...
        <div className="mt-auto flex items-center justify-end gap-2 border-t border-border bg-card/90 px-6 py-4 backdrop-blur">
          {o.accepted && !result ? (
            <>
